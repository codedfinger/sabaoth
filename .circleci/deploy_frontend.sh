...
ssh -o StrictHostKeyChecking=no -i $SSH_KEY ubuntu@$EC2_INSTANCE_IP "sudo mkdir -p /var/www/html"

# Connect to the EC2 instance and restart the Express app
scp -o StrictHostKeyChecking=no -i $SSH_KEY -r frontend/build/* ubuntu@$EC2_INSTANCE_IP:/var/www/html/
