...
ssh -o StrictHostKeyChecking=no -i -v ubuntu@"$INSTANCE_IP" "sudo mkdir -p /var/www/html"

# Connect to the EC2 instance and restart the Express app
scp -o StrictHostKeyChecking=no -i -r frontend/build/* ubuntu@"$INSTANCE_IP":/var/www/html/
