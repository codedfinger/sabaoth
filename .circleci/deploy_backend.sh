...
scp -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa -r backend/* ubuntu@"$INSTANCE_IP":/home/ubuntu/backend

# Connect to the EC2 instance and restart the Express app
ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa -v ubuntu@"$INSTANCE_IP" "cd /home/ubuntu/backend && npm install && pm2 start index.js"
