...
echo "$SSH_PRIVATE_KEY" > sabaoth.pem

chmod 400 sabaoth.pem

# Copy the built files to the EC2 instance
scp -o StrictHostKeyChecking=no -i sabaoth.pem -r backend/* ubuntu@"$INSTANCE_IP":/home/ubuntu/backend

# Connect to the EC2 instance and restart the Express app
ssh -o StrictHostKeyChecking=no -i sabaoth.pem -v ubuntu@"$INSTANCE_IP" "cd /home/ubuntu/backend && npm install && pm2 start index.js"
