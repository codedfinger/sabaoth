        ...
        stage('Build Image - Frontend') {
            steps {
                dir('frontend') {
                    script  {
                        sshagent(['skey']) {
                            sh "scp -r * ubuntu@44.203.35.206:/home/ubuntu/"
                        }
                    }
                }
            }
            ...
            steps {
                script {                    
                    // Copy the files to the remote server
                     sshagent(['skey']) {                        
                        sh "ssh -o StrictHostKeyChecking=no ubuntu@44.203.35.206 'sudo cp -r * /var/www/html'"
                        // sh "scp -r * ubuntu@3.88.152.217:/var/www/html"
                    }
                    
                    // Set appropriate permissions on the remote server
                    sshagent(['skey']) {
                        sh "ssh -o StrictHostKeyChecking=no ubuntu@44.203.35.206 'sudo chown -R www-data:www-data /var/www/html'"
                        sh "ssh -o StrictHostKeyChecking=no ubuntu@44.203.35.206 'sudo chmod -R 755 /var/www/html'"
                    }
                    
                    // Restart Apache on the remote server
                    sshagent(['skey']) {
                        sh "ssh -o StrictHostKeyChecking=no ubuntu@44.203.35.206 'sudo service nginx restart'"
                    }
                }
            }
        }
